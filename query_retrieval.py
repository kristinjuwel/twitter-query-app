...
def tweets(query, since, until):
    ...
    line = []
    with open ('frequency_result.csv', 'rt', encoding="utf8") as csvfile:
        reader=csv.reader(csvfile)
        i=0
        for row in reader:
            if (i>9):
    ...
    string0="\n1) " + ' - '.join([str(item) for item in line[0]])
    string1="\n2) " + ' - '.join([str(item) for item in line[1]])
    string2="\n3) " + ' - '.join([str(item) for item in line[2]])
    string3="\n4) " + ' - '.join([str(item) for item in line[3]])
    string4="\n5) " + ' - '.join([str(item) for item in line[4]])
    string5="\n6) " + ' - '.join([str(item) for item in line[5]])
    string6="\n7) " + ' - '.join([str(item) for item in line[6]])
    string7="\n8) " + ' - '.join([str(item) for item in line[7]])
    string8="\n9) " + ' - '.join([str(item) for item in line[8]])
    string9="\n10) " + ' - '.join([str(item) for item in line[9]])
    
    string_final = string0+ string1+ string2+ string3+ string4+ string5+ string6+ string7+ string8+ string9
